...
namespace pcl_filter_ns
{
    ...
    private:
        ...
        bool debug, sim, if_build_map = true;
        ...
        virtual void onInit()
        {
            ...
            nh.getParam("/IfBuildMap", if_build_map);
            nh.getParam("/WorldFrame", world_frame_id);
            ...
            //   resolution = 0.2;
            shrink_factor = -0.1;
            obj_timegap_max = 0.2; // max latency to receive obj message
            // voxel_id_list.clear();
            voxel_id_map.clear();
            ...
            odom_sub.subscribe(nh, "/piv/odom", 50);
            ...
            int n[4] = {static_cast<int>(bbox_size(0)), static_cast<int>(bbox_size(1)), static_cast<int>(bbox_size(2)), static_cast<int>(bbox_size(3))};
        ...
        void pcl_static_pb()
        {
            pcl_sta.header.frame_id = world_frame_id;
        ...
        int getObjRelaSeq(auto &point)
        {
            ...
            float min_dis2center = 2000;
            int match_id = 0;
            for (size_t i = 0; i < obj.bounding_boxes.size(); i++)
            {
                if (img_x < obj.bounding_boxes[i].xmax - obj_shrink[i](0) &&
                    img_y < obj.bounding_boxes[i].ymax - obj_shrink[i](1) &&
                    img_x > obj.bounding_boxes[i].xmin + obj_shrink[i](0) &&
                    img_y > obj.bounding_boxes[i].ymin + obj_shrink[i](1))
                {
                    float temp_dis = (img_x - (obj.bounding_boxes[i].xmax + obj.bounding_boxes[i].xmin) / 2) / (obj.bounding_boxes[i].xmax - obj.bounding_boxes[i].xmin) +
                     (img_y - (obj.bounding_boxes[i].ymax + obj.bounding_boxes[i].ymin) / 2) / (obj.bounding_boxes[i].ymax - obj.bounding_boxes[i].ymin);
                    if (temp_dis < min_dis2center)
                    {
                        min_dis2center = temp_dis;
                        match_id = i;
                    }
                }
            }
            if (min_dis2center < 1000)
                return match_id;
            else
            {
                cout << "this cluster fail to match, point: " << point << endl;
                return -1;
            }
        }
                 ...
                 const sensor_msgs::Imu::ConstPtr &imu_msg, const nav_msgs::Odometry::ConstPtr &odom_msg)
        {
            ...
            for (size_t idd = 0; idd < pcl_sta.points.size() && if_build_map; idd++)
            {
                ...
                {
                    pcl_sta.points.erase(pcl_sta.points.begin() + idd);
            ...
            if (if_build_map && pcl_sta.points.size() == 0)
            {
                voxel_id_map.clear();
            }
            if (dt < 0)
            ...
            if (debug)
            {
                cout << "this and last pcl frame size: " << pcl_list.back()->points.size() << " " << pcl_list.front()->points.size() << endl;
                cout << "dt: " << dt << endl;
            }
            if (dt > min_dt && pcl_list.back()->points.size() != 0 && pcl_list.front()->points.size() != 0)
            {
                ...
                if (debug)
                {
                    ...
                    cout << "time gap to obj: " << abs((time_list.back() - obj.header.stamp).toSec()) << "  if timeout: " << obj_timeout << endl;
                }
                ...
                // cout << "cluster number: " << index.size() << endl;
                for (auto id : index) //(auto &point:cloud->points)
                {
                    ...
                    for (auto pt_id = cluster_begin_idx[id]; pt_id < cluster_begin_idx[id] + cluster_size[id]; pt_id++)
                    {
                        ...
                        if (obj_timeout || !obj_updated || static_cast<size_t>(id) == cluster_1st.size() - 1) // ignore ground cluster
                            temp_ind = -1;
                        else if (pt_id == cluster_begin_idx[id])
                        {
                            ...
                            if (temp_ind >= 0)
                            {
                                for (auto id2 : index)
                                {
                                    if (id2 >= id)
                                        break;
                                    if ((point2img(cluster_1st[id2]) - point2img(cluster_1st[id])).norm() < 20) // depth of id2 always smaller (closer) than id
                                    {
                                        temp_ind = -1;
                                        ROS_WARN("found occluded object!");
                                        break;
                                    }
                                }
                            }
                        ...
                        else if (temp_ind >= 0)
                        {
                            ...
                            {
                                ROS_WARN("This cluster cannot be pushed inside. temp_size: %d", temp_size);
                            }
                ...
                if (if_build_map && pcl_sta.points.size() > static_cast<size_t>(pcl_sta_len))
                {
                    for (auto jj = pcl_sta.points.size() - 1; jj >= static_cast<size_t>(pcl_sta_len); jj--)
                    {
                ...
                if (dynamic)
                {
                    ...
                    for (size_t j = 0; j < xyz_clusters1.size(); j++)
                    {
                        if (debug)
                            cout << "size of cluser of frame 1 and 2: " << xyz_clusters1[j].size() << "  " << xyz_clusters2[j].size() << endl;
                        if (xyz_clusters1[j].size() > 5 && xyz_clusters2[j].size() > 5 && abs(1 - double(xyz_clusters1[j].size() / xyz_clusters2[j].size())) < 0.5)
                        {
            ...
            // std::cout << "Time range in message list: " << (time_list.back() - time_list.front()).toSec() << endl;
            for (size_t i = 0; i < KF_list.size() && debug; i++)
            {
                if (debug)
                {
            ...
            if (KF_list.size() + obj_num)
                cout << "\033[32mImg dynamic target num: " << obj_num << " KF size: " << KF_list.size() << "\033[0m" << endl;
        ...
        Eigen::Vector3d getDisp(Eigen::Vector3d target_disp, Eigen::Vector3d max_disp) //(int in1, int in2)
        {
            ...
            for (auto &ind : index)
            {
                if (ind.norm() < (max_disp.norm() + 5) && (target_disp.sum() == 0 || (ind - target_disp).norm() < 4) ) //
                {
                    dp = ind;
                    leagal = true;
                    break;
            ...
            if (debug)
                cout << "peak index: \n"
                     << dp << endl;
            int count = 1;
            for (auto i = start_id + 1; i < peak_num; i++)
            {
                if (((index[start_id] - index[i]).array().abs() < 2).all() && abs(peaks[start_id] - peaks[i]) / peaks[start_id] < 0.05)
                {
                    dp = dp + index[i];
                    count++;
                }
